...
bswf.carManEntry.inChargeOfOperationDeputyGeneralManagerCheckForm = {
	init : function(option,readonly){
		...
		$form.find(":input[name='rhanding']").change(function(){
			switch($(this).val()){
				case "送招聘专员归档":
					$form.find(":input[name='isChairmanCheck']").val(false);
				...
				case "送相关部门协办":
					...
					handing_hide($form.find("#confirmResult"));
					 break;
				case "送董事长审批":
					$form.find(":input[name='isChairmanCheck']").val(true);
					$form.find(":input[name='view']").attr("data-validate","required");
					handing_hide($form.find("#co4department"));
					handing_hide($form.find("#confirmResult"));
					 break;
		...
		function handing_show($div){
			...
			//css:bswf-carManEntry-flow
			var $flow=$div.find(".bswf-carManEntry-flow");
			$flow.each(function(){
					$(this).val(true);
			});
		}
