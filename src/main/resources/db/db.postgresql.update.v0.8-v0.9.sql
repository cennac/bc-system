...
ALTER TABLE BS_CASE_BASE ADD CONSTRAINT BSFK_CONTRACT_RECEIVER FOREIGN KEY (RECEIVER_ID)
...
ALTER TABLE BS_CASE_BASE ADD CONSTRAINT BSFK_CONTRACT_HANDLER FOREIGN KEY (HANDLER_ID)
      REFERENCES BC_IDENTITY_ACTOR_HISTORY (ID);

ALTER TABLE BS_CASE_BASE ADD COLUMN HANDLER_STATUS INTEGER;
COMMENT ON COLUMN BS_CASE_BASE.HANDLER_STATUS IS '处理状态';

ALTER TABLE BS_CASE_BASE ADD COLUMN HANDLER_OPINION VARCHAR(255);
COMMENT ON COLUMN BS_CASE_BASE.HANDLER_OPINION IS '处理意见';

ALTER TABLE BS_CASE_BASE ADD COLUMN HANDLER_DATE TIMESTAMP;
COMMENT ON COLUMN BS_CASE_BASE.HANDLER_DATE IS '处理时间';
