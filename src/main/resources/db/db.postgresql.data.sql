...
insert into BC_IDENTITY_RESOURCE (ID,STATUS_,INNER_,TYPE_,BELONG,ORDER_,NAME,URL,ICONCLASS) 
	select NEXTVAL('CORE_SEQUENCE'), 0, false, 2, m.id, '030303','司机迁移记录', '/bc/error/todo', 'i0401' from BC_IDENTITY_RESOURCE m where m.order_='030300';
...
insert into BC_IDENTITY_RESOURCE (ID,STATUS_,INNER_,TYPE_,BELONG,ORDER_,NAME,URL,ICONCLASS) 
	select NEXTVAL('CORE_SEQUENCE'), 0, false, 2, m.id, '031700','保险保单', '/bc/error/todo', 'i0802' from BC_IDENTITY_RESOURCE m where m.order_='030000';
insert into BC_IDENTITY_RESOURCE (ID,STATUS_,INNER_,TYPE_,BELONG,ORDER_,NAME,URL,ICONCLASS) 
	select NEXTVAL('CORE_SEQUENCE'), 0, false, 2, m.id, '031800','费用', '/bc/error/todo', 'i0800' from BC_IDENTITY_RESOURCE m where m.order_='030000';
...
-- 插入费用管理角色数据
insert into  BC_IDENTITY_ROLE (ID, STATUS_,INNER_,TYPE_,ORDER_,CODE,NAME) 
	values(NEXTVAL('CORE_SEQUENCE'), 0, false,  0,'0114', 'BS_COST','费用管理');
insert into BC_IDENTITY_ROLE_RESOURCE (RID,SID) 
...
BEGIN
	for i in 1..maxrecords loop
		insert into BC_OPTION_ITEM (ID,STATUS_, PID, ORDER_, KEY_, VALUE_, ICON) 
			select NEXTVAL('CORE_SEQUENCE'), 0, g.id, i, i, i, null from BC_OPTION_GROUP g where g.KEY_='motorcade.paymentDate';  
	end loop;
END;
$$ LANGUAGE plpgsql;
