...
CREATE TABLE BS_CAR_DRIVER_HISTORY (
   ...
   STATUS_              NUMBER(1)            NOT NULL,
   DRIVER_ID            NUMBER(19)  ,
   OLD_CAR_ID             NUMBER(19),
   OLD_MOTORCADE_ID       NUMBER(19),
   OLD_DRIVER_STATE       NUMBER(1),
   NEW_CAR_ID             NUMBER(19),
   NEW_MOTORCADE_ID       NUMBER(19),
   NEW_DRIVER_STATE       NUMBER(1),
   MOVE_DATE              DATE,
   ...
   MODIFIER_ID          NUMBER(19),
   DESC_                VARCHAR2(4000),
  CONSTRAINT BS_CAR_DRIVER_HISTORY PRIMARY KEY (ID)
);
COMMENT ON TABLE BS_CAR_DRIVER_HISTORY IS '人车关系';
COMMENT ON COLUMN BS_CAR_DRIVER_HISTORY.STATUS_ IS '状态：0-启用中,1-已禁用,2-已删除';
COMMENT ON COLUMN BS_CAR_DRIVER_HISTORY.DRIVER_ID IS '司机ID';
COMMENT ON COLUMN BS_CAR_DRIVER_HISTORY.OLD_CAR_ID IS '原车辆ID';
COMMENT ON COLUMN BS_CAR_DRIVER_HISTORY.OLD_MOTORCADE_ID IS '原车队ID';
COMMENT ON COLUMN BS_CAR_DRIVER_HISTORY.OLD_DRIVER_STATE IS '营运班次:如1-正班、2-副班、3-顶班';
COMMENT ON COLUMN BS_CAR_DRIVER_HISTORY.NEW_CAR_ID IS '新车辆ID';
COMMENT ON COLUMN BS_CAR_DRIVER_HISTORY.NEW_MOTORCADE_ID IS '新车队ID';
COMMENT ON COLUMN BS_CAR_DRIVER_HISTORY.NEW_DRIVER_STATE IS '营运班次:如1-正班、2-副班、3-顶班';
...
ALTER TABLE BS_CAR_DRIVER_HISTORY ADD CONSTRAINT BSFK_CARDRIVERHISTORY_MODIFIER FOREIGN KEY (MODIFIER_ID)
      REFERENCES BC_IDENTITY_ACTOR_HISTORY (ID);
ALTER TABLE BS_CAR_DRIVER_HISTORY ADD CONSTRAINT BSFK_CARDRIVER_OLD_CAR FOREIGN KEY (OLD_CAR_ID)
      REFERENCES BS_CAR (ID);
ALTER TABLE BS_CAR_DRIVER_HISTORY ADD CONSTRAINT BSFK_CARDRIVER_NEW_CAR FOREIGN KEY (NEW_CAR_ID)
      REFERENCES BS_CAR (ID);
ALTER TABLE BS_CAR_DRIVER_HISTORY ADD CONSTRAINT BSFK_CASEBASE_OLD_MOTORCADE FOREIGN KEY (OLD_MOTORCADE_ID)
      REFERENCES BS_MOTORCADE (ID);
ALTER TABLE BS_CAR_DRIVER_HISTORY ADD CONSTRAINT BSFK_CASEBASE_NEW_MOTORCADE FOREIGN KEY (NEW_MOTORCADE_ID)
      REFERENCES BS_MOTORCADE (ID);
ALTER TABLE BS_CAR_DRIVER_HISTORY ADD CONSTRAINT BSFK_CARDRIVERHISTORY_CARMAN FOREIGN KEY (DRIVER_ID)
      REFERENCES BS_CARMAN (ID);
      CREATE INDEX BSIDX_CARDRIVERHISTORY_CAR ON BS_CAR_DRIVER_HISTORY (NEW_CAR_ID);
...
COMMENT ON COLUMN BS_SYNC_JIAOWEI_JTWF.CAR_PLATE IS '车牌号码';
...
ALTER TABLE BS_SYNC_JIAOWEI_JTWF ADD CONSTRAINT BSFK_JIAOWEIJTWF2BASE FOREIGN KEY (ID)
      REFERENCES BC_SYNC_BASE (ID);
CREATE INDEX BCIDX_JIAOWEIJTWF_CERT ON BS_SYNC_JIAOWEI_JTWF (DRIVER_CERT);
