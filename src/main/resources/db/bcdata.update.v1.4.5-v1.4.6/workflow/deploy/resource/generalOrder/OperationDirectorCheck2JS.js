...
bswf.generalOrder.OperationDirectorCheckForm = {
		...
		$form.find(":input[name='rhanding']").change(function(){
			switch($(this).val()){
				case "送部门经理确认" :
					...
					if(!$form.find(":input[name='is_ODC_to_GMGC_flow']").hasClass("ignore")){
						$form.find(":input[name='is_ODC_to_GMGC_flow']").addClass("ignore");
					}
					 break;
				case "送总经理组审批":
					handing_show($form.find("#GeneralManagerGroupCheck"));
					if($form.find(":input[name='is_ODC_to_GMGC_flow']").hasClass("ignore")){
					   $form.find(":input[name='is_ODC_to_GMGC_flow']").removeClass("ignore");
					}
					 break;
				default: alert("other");
			}
	...
	buildFormData : function(){
		...
		if(rhanding=="送总经理组审批"){
			...
			$form.find(":input[name='list_gmgc4assignee4local']").val($.toJSON(list_departmentAndAssignee));
		}
	...
	validateForm: function(){
		...
		bswf.generalOrder.OperationDirectorCheckForm.buildFormData.call(this);
