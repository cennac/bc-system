...
CREATE TABLE BC_WF_ATTACH(
	...
	MODIFIED_DATE	TIMESTAMP,
	MODIFIER_ID	INTEGER,
	CONSTRAINT BCPK_WF_ATTACHE PRIMARY KEY (ID)
);
COMMENT ON TABLE BC_WF_ATTACH IS '流程附加信息';
COMMENT ON COLUMN BC_WF_ATTACH.TASK_ID_ IS '流程任务id';
COMMENT ON COLUMN BC_WF_ATTACH.PROC_INST_ID_ IS '流程实例id';
COMMENT ON COLUMN BC_WF_ATTACH.TYPE_ IS '类型：1-附件，2-意见';
COMMENT ON COLUMN BC_WF_ATTACH.COMMON IS '是否为公共信息，true是，false任务信息';
COMMENT ON COLUMN BC_WF_ATTACH.SUBJECT IS '附件标题';
COMMENT ON COLUMN BC_WF_ATTACH.PATH_ IS '附件路径';
COMMENT ON COLUMN BC_WF_ATTACH.EXT_ IS '附件扩展名';
