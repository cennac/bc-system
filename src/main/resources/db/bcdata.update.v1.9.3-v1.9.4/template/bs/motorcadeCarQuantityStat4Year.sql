select m.id mid, u.name uname, m.name mname, q.year_ as year, to_char(avg(q.quantity),'FM999.0') quantity
...
group by u.id,u.name,m.id,m.name,q.year_
order by q.year_,m.code